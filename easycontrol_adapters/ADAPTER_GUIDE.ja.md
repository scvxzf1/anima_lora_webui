...
| 4 | `scripts/experimental_tasks/{training,inference}.py` | `_EASYADAPTERS = {"colorize"}` + 分岐 | `EASYADAPTER=<task>` を `make exp-easycontrol*` コマンドで動くようにする |
...
`make exp-easycontrol*` コマンドは `EASYADAPTER` 環境変数で切り替えます。三つの小さな編集で `EASYADAPTER=<task>` が自分の config、prep、チェックポイントを使うようになります。

**`scripts/experimental_tasks/training.py` で:**
...
**`scripts/experimental_tasks/inference.py`** (`cmd_test_easycontrol`) で: セレクターは現在 colorize をハードコードしています。自分のタスク向けにいくつかの colorize 専用の値を汎用化してください — チェックポイント名、出力フォルダ、フォールバック参照フォルダ、空プロンプトのデフォルト:
