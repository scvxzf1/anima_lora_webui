...
한 줄이면 됩니다 — [uv](https://astral.sh/uv)가 없으면 설치하고, 최신 릴리스를 받아 `uv sync`까지 실행합니다:
...
`./anima_lora/`에 설치됩니다 (`ANIMA_DIR`로 경로 변경, `ANIMA_VERSION=v1.4.0`으로 특정 태그 고정 가능). Windows에서는 바탕화면에 **"Anima LoRA GUI"** 바로가기도 생성됩니다. 이후 인증하고 모델을 받습니다:
...
이후 업데이트는 `make update` (릴리스 tarball 병합, git 불필요).
