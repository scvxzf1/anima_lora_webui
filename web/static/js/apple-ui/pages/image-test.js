...
import { mergedConfigUrl, loadTrainingContext } from './training-controls.js?v=apple-ui-20260812v33';
...
function renderPage(state, images) {
    ...
    return `
        <div class="apple-page apple-page-wide apple-image-test-page">
            <div class="apple-page-hero apple-reveal"><span class="apple-eyebrow">模型验证</span><h1>生图测试</h1><p>使用当前训练配置和已生成权重运行一次真实推理。</p></div>
            <form class="apple-image-test-form apple-reveal" data-image-test-form data-stagger="1">
                <div class="apple-image-test-primary">
                ...
                <div class="apple-dataset-field-grid">
                    ...
                    <label class="apple-field"><span class="apple-field-label-text">计算设备</span><select class="apple-select" data-image-field="gpu_index"><option value="">自动选择</option>${gpuOptions}</select></label>
                    <label class="apple-field"><span class="apple-field-label-text">权重文件</span><select class="apple-select" data-image-field="weight_path"><option value="">使用基础模型</option>${weightOptions}</select></label>
                    ${textField('save_path', '输出目录', savePath)}
                </div>
                ${renderSelectiveLora()}
                <div class="apple-config-actions"><button class="apple-btn apple-btn-primary" type="submit" data-image-action="start" ${state.running ? 'disabled' : ''}>${state.running ? '推理进行中' : '开始推理'}</button>${state.running ? '<button class="apple-btn apple-btn-secondary" type="button" data-image-action="stop">停止推理</button>' : ''}</div>
                <p class="apple-config-feedback" data-image-feedback role="status" aria-live="polite">${escapeHtml(status.error || '')}</p>
            </form>
            <section class="apple-section apple-reveal" data-stagger="2"><div class="apple-section-header-row"><div><span class="apple-eyebrow">输出结果</span><h2 class="apple-section-title">推理预览</h2></div><span class="apple-section-desc">${images.length} 张图片</span></div>${images.length ? `<div class="apple-image-grid">${images.map(renderImage).join('')}</div>` : '<div class="apple-empty-state"><p>暂无生成图片</p></div>'}</section>
        </div>
    `;
}
...
function bindPage(root, state) {
    ...
    if (state.running) {
        ...
        pollTimer = window.setTimeout(() => window.dispatchEvent(new CustomEvent('apple-refresh-route')), 3000);
    }
}
...
function renderSelectiveLora() {
    return `<details class="apple-dataset-advanced apple-image-lora-details"><summary>分层 LoRA 加载</summary><div class="apple-selective-lora-head"><label class="apple-check-field"><input type="checkbox" data-image-field="anima_selective_lora"><span>启用分层加载</span></label>${selectField('anima_selective_preset', '层位预设', 'default', LORA_PRESETS)}</div><div data-selective-lora-fields hidden><p class="apple-section-desc">每个层位可独立关闭或设置 0 至 2 倍强度。</p><div class="apple-lora-block-grid">${LORA_BLOCKS.map(([key, label]) => `<label class="apple-lora-block"><span>${label}</span><input class="apple-input" type="number" min="0" max="2" step="0.05" value="1" data-lora-block="${key}"></label>`).join('')}</div></div></details>`;
}
...
function numberField(key, label, value, step = '1') { return `<label class="apple-field"><span class="apple-field-label-text">${label}</span><input class="apple-input" type="number" step="${step}" data-image-field="${key}" value="${escapeAttribute(value)}"></label>`; }
function textField(key, label, value) { return `<label class="apple-field"><span class="apple-field-label-text">${label}</span><input class="apple-input" type="text" data-image-field="${key}" value="${escapeAttribute(value)}"></label>`; }
function selectField(key, label, value, options) { return `<label class="apple-field"><span class="apple-field-label-text">${label}</span><select class="apple-select" data-image-field="${key}">${options.map(([option, text]) => `<option value="${escapeAttribute(option)}" ${String(option) === String(value) ? 'selected' : ''}>${escapeHtml(text)}</option>`).join('')}</select></label>`; }
function textareaField(key, label, value, placeholder) { return `<label class="apple-field apple-field-wide"><span class="apple-field-label-text">${label}</span><textarea class="apple-textarea" data-image-field="${key}" placeholder="${placeholder}">${escapeHtml(value)}</textarea></label>`; }
...
function renderImage(image) { return `<figure class="apple-image-card"><img src="${escapeAttribute(image.url || '')}" alt="${escapeAttribute(image.name || '')}" loading="lazy"><figcaption>${escapeHtml(image.prompt || image.name || '')}</figcaption></figure>`; }
function showFeedback(root, message, tone) { const el = root.querySelector('[data-image-feedback]'); if (el) { el.textContent = message; el.dataset.tone = tone; el.classList.add('apple-config-feedback-visible'); } }
