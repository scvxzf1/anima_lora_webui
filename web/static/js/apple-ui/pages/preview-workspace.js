...
export async function loadPreviewWorkspace() {
    ...
    return `
        <div class="apple-page apple-page-wide apple-preview-page">
            <div class="apple-page-hero apple-reveal">
                <h1>预览工作区</h1>
                <p>查看训练过程实际生成的样张与采样信息。</p>
            </div>

            <div class="apple-preview-summary apple-reveal" data-stagger="1">
                ...
                <div><span>样张目录</span><strong class="apple-text-mono">${escapeHtml(directory || '未设置')}</strong></div>
            </div>

            ${images.length ? `
                <section class="apple-section apple-reveal" data-stagger="2">
                    <div class="apple-section-header-row">
                        <div><span class="apple-eyebrow">训练样张</span><h2 class="apple-section-title">最近生成</h2></div>
                        <p class="apple-section-desc">共 ${Number(listing?.total || images.length).toLocaleString('zh-CN')} 张</p>
                    </div>
                    <div class="apple-image-grid">
                        ${images.map(renderImage).join('')}
                    </div>
                </section>
            ` : `
                <div class="apple-empty-state apple-reveal" data-stagger="2"><p>${escapeHtml(message)}</p></div>
            `}
        </div>
    `;
}
...
function renderImage(image) {
    ...
    return `
        <figure class="apple-image-card">
            <img src="${escapeAttribute(image.url || '')}" alt="${escapeAttribute(image.name || title)}" loading="lazy">
            <figcaption class="apple-image-card-caption"><strong>${escapeHtml(title)}</strong>${details ? `<span>${escapeHtml(details)}</span>` : ''}</figcaption>
            ${prompt ? `<div class="apple-image-card-prompt">${escapeHtml(prompt)}</div>` : ''}
        </figure>
    `;
}
