...
export async function loadGlobalSettings() {
    ...
    try {
        ...
        return `<div class="apple-page"><div class="apple-page-hero"><h1>全局设置</h1></div><div class="apple-empty-state"><p>${escapeHtml(error.message || '读取全局设置失败')}</p></div></div>`;
    }
...
function renderPage(settings) {
    return `
        <div class="apple-page apple-page-wide apple-global-settings-page">
            <div class="apple-page-hero apple-reveal">
                <span class="apple-eyebrow">模型与系统</span>
                <h1>全局设置</h1>
                <p>管理本机存储位置、推理输出和界面缩放。模型路径在“全局模型配置”中维护。</p>
            </div>
            <form class="apple-global-settings-form" data-global-settings-form>
                ...
                <div class="apple-config-actions apple-config-actions-sticky">
                    <button class="apple-btn apple-btn-primary" type="submit" data-global-action="save">保存全局设置</button>
                    <span class="apple-config-feedback" data-global-feedback role="status" aria-live="polite"></span>
                </div>
            </form>
        </div>
    `;
...
function renderGroup(group, settings, index) {
    return `
        <section class="apple-config-entry apple-reveal" data-stagger="${Math.min(index + 1, 6)}">
            <header class="apple-config-entry-header"><span class="apple-eyebrow">${group.eyebrow}</span><h2>${group.title}</h2><p>${group.desc}</p></header>
            <div class="apple-config-entry-fields"><div class="apple-field-grid-2">${group.fields.map((field) => renderField(field, settings[field[0]])).join('')}</div></div>
        </section>
    `;
}
...
function renderField([key, label, type, placeholder], value) {
    if (type === 'boolean') {
        return `<div class="apple-field"><div class="apple-toggle-row"><button class="apple-toggle" type="button" data-global-key="${key}" data-checked="${Boolean(value)}" role="switch" aria-checked="${Boolean(value)}"></button><div><div class="apple-toggle-label">${label}</div><div class="apple-toggle-desc">默认关闭，只扫描训练器管理的输出路径。</div></div></div></div>`;
    }
    const minMax = type === 'number' ? ' min="25" max="400" step="5"' : '';
    return `<label class="apple-field"><span class="apple-field-label-text">${label}</span><input class="apple-input" type="${type}" data-global-key="${key}" value="${escapeAttribute(value ?? '')}" placeholder="${escapeAttribute(placeholder)}"${minMax}></label>`;
}

function bindPage(root) {
    root.querySelectorAll('.apple-toggle[data-global-key]').forEach((toggle) => {
...
async function saveSettings(root) {
    ...
    try {
        ...
        root.querySelectorAll('[data-global-key]').forEach((field) => {
            ...
            if (field.classList.contains('apple-toggle')) values[key] = field.dataset.checked === 'true';
...
function showFeedback(root, message, tone) { const el = root.querySelector('[data-global-feedback]'); if (el) { el.textContent = message; el.dataset.tone = tone; el.classList.add('apple-config-feedback-visible'); } }
