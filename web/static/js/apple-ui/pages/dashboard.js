 ...
 * Keeps the Apple hierarchy while bringing the classic live monitor into the
...
import { calculateTrainingEtaMetricInfo, formatLr } from '../../features/live-training/index.js?v=apple-ui-20260812v33';
import { renderLossChart } from './live-training.js?v=apple-ui-20260812v33';
import { appendLogLine, connectWebSocket, disconnectWebSocket, onMessage } from '../ws.js?v=apple-ui-20260812v33';
...
function renderDashboard(data) {
    ...
    const historyHtml = model.history.length
        ? `<ul class="apple-history-list">${model.history.map(renderHistoryItem).join('')}</ul>`
        : '<div class="apple-empty-state"><p>暂无训练记录</p></div>';
    const logsHtml = model.logs.length
        ? model.logs.map(renderLogRecord).join('')
        : '<div class="apple-empty-state"><p>训练开始后显示实时日志</p></div>';

    return `
        <div class="apple-page apple-page-wide apple-dashboard-page" data-dashboard-root>
            <div class="apple-dashboard-hero apple-reveal">
                <div class="apple-dashboard-hero-copy">
                    <div class="apple-dashboard-hero-meta">
                        <span class="apple-eyebrow">训练工作台</span>
                        <div class="apple-status-badge" data-dashboard-state data-state="${model.state}">
                            <span class="apple-nav-status-dot" data-state="${model.state}"></span>
                    ...
                    <div class="apple-dashboard-command-row">
                        ${commandButton('activity', '查看实时训练', 'live', 'apple-btn-primary')}
                        ${commandButton('settings', '调整训练配置', 'config', 'apple-btn-secondary')}
                        ${commandButton('stop', '停止训练', 'stop', 'apple-btn-secondary', !model.running)}
                    </div>
                </div>
                <div class="apple-dashboard-progress-visual" data-dashboard-ring style="--dashboard-progress: ${model.progressPercent}%">
                    <div class="apple-dashboard-progress-ring">
                        <div class="apple-dashboard-progress-ring-center">
                            <strong data-dashboard-progress-percent>${Math.round(model.progressPercent)}%</strong>
                            <span>训练进度</span>
                        </div>
                    </div>
                    <div class="apple-dashboard-ring-details">
            ...
            <section class="apple-dashboard-runband apple-reveal" data-stagger="1">
            ...
            <section class="apple-dashboard-progress-section apple-reveal" data-stagger="2">
                ${sectionHeading('gauge', '训练进度', '当前训练量', `<strong data-dashboard-progress-text>${escapeHtml(model.progressText)}</strong>`)}
                <div class="apple-dashboard-progress-track" aria-label="训练进度">
                    <div class="apple-dashboard-progress-fill" data-dashboard-progress-fill style="width: ${model.progressPercent}%"></div>
                </div>
            </section>

            <section class="apple-dashboard-overview apple-reveal" data-stagger="3" aria-label="训练关键概览">
            ...
            <section class="apple-dashboard-monitor-section apple-reveal" data-stagger="3">
                ${sectionHeading('activity', '实时指标', '训练参数变化', '<span class="apple-dashboard-section-note">当前训练输出</span>')}
                <div class="apple-dashboard-metric-grid">
                ...
                <div class="apple-dashboard-resource-grid">
            ...
            <section class="apple-dashboard-chart-section apple-reveal" data-stagger="4">
                ${sectionHeading('chart', '趋势', '损失值 / 学习率曲线', `<span class="apple-dashboard-section-note" data-dashboard-chart-count>最近 ${model.metrics.length} 个点</span>`)}
                <div class="apple-dashboard-chart" data-dashboard-chart>${renderLossChart(model.metrics)}</div>
            </section>

            <div class="apple-dashboard-lower-grid">
                <section class="apple-section apple-dashboard-history-section apple-reveal" data-stagger="5">
                    ${sectionHeading('history', '记录', '最近训练', `<button class="apple-inline-action" type="button" data-dashboard-action="history">查看全部</button>`)}
                    ${historyHtml}
                </section>
                <section class="apple-section apple-dashboard-log-section apple-reveal" data-stagger="6">
                    ${sectionHeading('terminal', '输出', '实时日志', `<span class="apple-dashboard-section-note" data-dashboard-log-status>${model.running ? '实时连接中' : '等待训练'}</span>`)}
                    <div class="apple-log-stream apple-dashboard-log-stream" data-dashboard-log>${logsHtml}</div>
                </section>
            </div>
        </div>
    `;
...
function commandButton(icon, label, action, variant, hidden = false) {
    return `<button class="apple-btn ${variant}" type="button" data-dashboard-action="${action}" ${hidden ? 'hidden' : ''}>${renderIcon(icon, 'apple-btn-icon')}<span>${label}</span></button>`;
}

function contextItem(icon, label, value, key) {
    return `
        <div class="apple-dashboard-context-item">
            <span class="apple-dashboard-context-icon">${renderIcon(icon)}</span>
            <div>
                <span class="apple-dashboard-kicker">${label}</span>
...
function sectionHeading(icon, eyebrow, title, trailing = '') {
    return `
        <div class="apple-dashboard-section-head">
            <div class="apple-dashboard-heading-copy">
                <span class="apple-dashboard-heading-icon">${renderIcon(icon)}</span>
                <div><span class="apple-eyebrow">${eyebrow}</span><h2>${title}</h2></div>
            </div>
            ${trailing}
        </div>
    `;
...
function overviewItem(icon, label, value, detail, key, tone = 'neutral') {
    return `
        <div class="apple-dashboard-overview-item" data-tone="${tone}">
            <span class="apple-dashboard-overview-icon" data-dashboard-summary-icon="${key}">${renderIcon(icon)}</span>
            <div>
                <span class="apple-dashboard-overview-label">${label}</span>
...
function metricCell(key, label, value) {
    ...
    return `
        <div class="apple-dashboard-metric-cell" data-dashboard-metric="${key}">
            <span class="apple-dashboard-metric-label">${renderIcon(icons[key] || 'activity')}<span>${label}</span></span>
...
function bindDashboardActions(wrapper, model) {
    wrapper.querySelectorAll('[data-dashboard-action]').forEach((button) => {
        button.addEventListener('click', async () => {
            ...
            if (action === 'stop') {
                ...
                button.innerHTML = `${renderIcon('stop', 'apple-btn-icon')}<span>正在停止</span>`;
    ...
    wrapper.querySelectorAll('.apple-history-item').forEach((item) => {
...
function renderDashboardState(wrapper, model) {
    ...
    const trendItem = wrapper.querySelector('[data-dashboard-summary="loss-trend"]')?.closest('.apple-dashboard-overview-item');
    ...
    const dot = wrapper.querySelector('[data-dashboard-state] .apple-nav-status-dot');
    ...
    if (stop) {
        ...
        stop.disabled = false;
        stop.innerHTML = `${renderIcon('stop', 'apple-btn-icon')}<span>停止训练</span>`;
    }
...
function appendDashboardLog(wrapper, text, level) {
    ...
    if (!log) return;
    log.querySelector('.apple-empty-state')?.remove();
...
function renderHistoryItem(task) {
    ...
    return `
        <li class="apple-history-item" data-task-id="${escapeHtml(task.id || '')}">
            <span class="apple-history-item-name">${escapeHtml(name)}</span>
            <span class="apple-history-item-state" data-state="${state}">${stateText(state)}</span>
            <span class="apple-history-item-meta">${escapeHtml(time)}</span>
        </li>
    `;
}
...
function renderLogRecord(record) {
    ...
    return `<div class="apple-log-line" data-level="${escapeHtml(level)}">${escapeHtml(text)}</div>`;
}
