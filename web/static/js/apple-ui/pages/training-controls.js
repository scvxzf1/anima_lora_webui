...
const STORAGE_KEY = 'anima_apple_training_context';
...
export function renderTrainingControls(context) {
    ...
    return `
        <section class="apple-config-runbar apple-reveal" aria-label="训练运行设置">
            <label class="apple-runbar-field apple-runbar-config"><span>当前训练配置</span><select class="apple-select" data-training-context="file">${fileOptions}</select></label>
            <label class="apple-runbar-field"><span>运行覆盖</span><select class="apple-select" data-training-context="preset">${presetOptions}</select></label>
            ${renderGpuPicker(context)}
            <div class="apple-runbar-actions">
                <button class="apple-btn apple-btn-secondary" type="button" data-training-action="preflight">${renderIcon('check', 'apple-btn-icon')}<span>训练前检查</span></button>
                <button class="apple-btn apple-btn-primary" type="button" data-training-action="start">${renderIcon('activity', 'apple-btn-icon')}<span>开始训练</span></button>
                <button class="apple-btn apple-btn-secondary" type="button" data-training-action="queue">${renderIcon('list', 'apple-btn-icon')}<span>加入队列</span></button>
            </div>
        </section>
        <dialog class="apple-training-dialog" data-training-dialog><div data-training-dialog-content></div></dialog>
    `;
}
...
function renderGpuPicker(context) {
    ...
    `).join('');
    return `<div class="apple-runbar-device"><span class="apple-runbar-label">训练设备</span><details class="apple-runbar-gpus" data-training-gpus><summary>${label}</summary><div class="apple-runbar-gpu-list"><button type="button" data-gpu-all>使用全部 GPU</button>${options || '<span>未读取到 GPU</span>'}</div></details></div>`;
}
...
function preflightLines(payload) {
    ...
    return `<ul class="apple-training-checks">${checks.map((item) => `<li data-level="${item.level || 'info'}">${item.message || item.key}</li>`).join('')}</ul>`;
}
...
function openDialog(root, { title, body, confirmText = '关闭', cancelText = '' }) {
    ...
    content.innerHTML = `<h2>${title}</h2><div>${body}</div><div class="apple-training-dialog-actions">${cancelText ? `<button class="apple-btn apple-btn-secondary" value="cancel">${cancelText}</button>` : ''}${confirmText ? `<button class="apple-btn apple-btn-primary" value="confirm">${confirmText}</button>` : '<button class="apple-btn apple-btn-primary" value="cancel">关闭</button>'}</div>`;
