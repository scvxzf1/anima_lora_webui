/* Navigation category map for the Apple-style UI.
 ...
 * Structure mirrors Apple's globalnav flyout: grouped columns with section headers.
 */

export const APPLE_NAV_CATEGORIES = [
    {
        id: 'training-config',
        label: '训练配置',
        layout: 'config',
...
export const APPLE_ALL_SUB_ITEMS = APPLE_NAV_CATEGORIES.flatMap((cat) =>
    (cat.groups || []).flatMap((group) =>
        group.items.map((sub) => ({ ...sub, categoryId: cat.id }))
    )
);

export const APPLE_CONFIG_CATEGORY_IDS = new Set([
    'training-config',
    'memory-optimization',
    'advanced-methods',
]);
...
export function isConfigCategory(categoryId) {
    return APPLE_CONFIG_CATEGORY_IDS.has(categoryId);
}

export function findSubItem(id) {
    return APPLE_ALL_SUB_ITEMS.find((item) => item.id === id);
}

export function findCategory(categoryId) {
    return APPLE_NAV_CATEGORIES.find((cat) => cat.id === categoryId);
}
