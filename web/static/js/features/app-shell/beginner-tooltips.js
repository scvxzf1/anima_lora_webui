/**
 * Beginner tooltip titles for main WebUI controls.
 */
...
export function installBeginnerTooltips() {
    const tips = {
        ...
        'btn-create-blank-preset': '以 LoRA 标准训练变体 lora.toml 为模板，并套用默认全局模型配置创建新的可编辑项目预设。',
        'btn-fill-global-model-paths': '从全局模型配置中选择一项，并填写模型格式、DiT、Qwen3 和 VAE。',
    ...
    'history-filter-model-family': '按历史任务使用的基座模型类型筛选。',
    'history-filter-training-variant': '按历史详情概览中的训练变体（方法族，如 lokr / lora）筛选。',
        ...
        'btn-save-global-settings': '保存输出目录、配置根目录和界面缩放；修改配置根目录后页面会自动刷新。',
        ...
        'btn-model-config-create': '新建一项可复用的 Anima 或 Krea-2 模型配置。',
        'btn-model-config-manage': '显示排序和删除操作；默认配置需要先切换后才能删除。',
    ...
    document.querySelectorAll('.tab-btn').forEach((btn) => {
        const labels = {
            ...
            settings: '全局设置页：管理输出目录、配置根目录和界面缩放。',
            'model-config': '全局模型配置页：管理可复用的 Anima 与 Krea-2 模型路径组合。',
