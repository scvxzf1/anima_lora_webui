...
export function renderProviderPanel(settings, routing) {
    return `<dialog class="dragon-caption-settings-dialog" data-caption-settings-dialog aria-labelledby="caption-provider-title"><section class="dragon-caption-section" data-caption-settings-section="provider">
        ...
        <form class="dragon-caption-provider-form" data-caption-provider-form>
            ...
            <label class="dragon-field"><span>API Key</span><div class="dragon-caption-secret-field"><input class="dragon-input" type="password" name="api_key" value="" autocomplete="new-password" placeholder="${settings.api_key_configured ? '留空以保留已保存密钥' : '输入 API Key'}"><button class="dragon-icon-button" type="button" data-toggle-api-key title="显示或隐藏 API Key" aria-label="显示或隐藏 API Key">◉</button></div></label>
...
export function bindProviderPanel(root, state) {
    ...
    root.querySelector('[data-toggle-api-key]')?.addEventListener('click', (event) => { const input = form?.elements.api_key; if (!input) return; input.type = input.type === 'password' ? 'text' : 'password'; event.currentTarget.setAttribute('aria-pressed', String(input.type === 'text')); });
...
async function runAction(root, state, action) {
    ...
    try {
        ...
        if (![draft.retry_count, draft.retry_interval_seconds, draft.concurrency, draft.timeout_seconds].every(Number.isFinite)) throw new Error('重试、并发和超时参数必须是有效数字');
