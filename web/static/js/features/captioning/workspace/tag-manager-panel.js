...
export function renderTagManagerPanel(state) {
    ...
    </div><div class="dragon-caption-split-list dragon-caption-tag-manager"><aside><input class="dragon-input" data-tag-search aria-label="搜索图片或 Tag" placeholder="搜索图片或 Tag" value="${escapeAttribute(query)}"><small data-tag-count>${visible.length}/${items.length} 项</small><div>${items.map((item) => `<button type="button" data-tag-item="${escapeAttribute(item.name)}" data-active="${item.name === state.workspaceData.tagItemName}" ${visible.includes(item) ? '' : 'hidden'}><strong>${escapeHtml(item.name)}</strong><small>${item.text_length} 字符</small></button>`).join('')}</div><div class="dragon-empty-state" data-tag-empty ${items.length && !visible.length ? '' : 'hidden'}><p>${items.length ? '没有匹配的图片或 Tag。' : '选择目录组并加载标注。'}</p>${!items.length ? '<button class="dragon-btn dragon-btn-secondary" type="button" data-tag-focus-group>选择目录组</button>' : ''}</div></aside><section>${state.workspaceData.tagNotice ? `<div class="dragon-caption-inline-status" role="status">${escapeHtml(state.workspaceData.tagNotice)}</div>` : ''}${selected ? renderEditor(selected, state) : `<div class="dragon-empty-state"><p>${items.length ? '选择一项开始编辑。' : '选择目录组并加载标注。'}</p></div>`}</section></div>`);
}

export function bindTagManagerPanel(root, state) {
    const groupSelect = root.querySelector('[data-tag-group]');
    root.querySelector('[data-tag-focus-group]')?.addEventListener('click', () => groupSelect?.focus());
    ...
    root.querySelector('[data-tag-search]')?.addEventListener('input', (event) => {
        ...
        const empty = root.querySelector('[data-tag-empty]'); if (empty) empty.hidden = visible > 0 || !state.workspaceData.tagItems.length;
    });
    root.querySelector('[data-tag-find]')?.addEventListener('input', (event) => { state.workspaceData.tagFind = event.target.value; const replace = root.querySelector('[data-tag-replace-all]'); if (replace) replace.disabled = !event.target.value || !state.workspaceData.tagItems.length; });
    ...
    root.querySelector('[data-tag-undo]')?.addEventListener('click', () => { const item = selectedItem(state); if (!item) return; delete state.workspaceData.tagDrafts?.[item.name]; state.workspaceData.tagDirtyName = ''; state.workspaceData.tagNotice = '已撤回当前未保存修改'; state.suiteRender(); });
