...
function renderPage(state) {
    return `
        <div class="dragon-page dragon-page-wide dragon-model-config-page">
            ...
            <div class="dragon-model-config-layout dragon-reveal" data-stagger="1">
                <aside class="dragon-model-config-sidebar">
                    <div class="dragon-section-header-row"><div><span class="dragon-eyebrow">配置库</span><h2 class="dragon-section-title">模型组合</h2></div><button class="dragon-btn dragon-btn-secondary dragon-btn-sm" type="button" data-model-action="add">新建</button></div>
                    <div class="dragon-model-config-list" data-model-list>${renderList(state)}</div>
                </aside>
                <form class="dragon-model-config-editor" data-model-form>${renderEditor(selectedItem(state), state)}</form>
            </div>
            <div class="dragon-config-actions dragon-config-actions-sticky">
                <button class="dragon-btn dragon-btn-secondary" type="button" data-model-action="default">设为默认</button>
                <button class="dragon-btn dragon-btn-secondary" type="button" data-model-action="remove">删除当前配置</button>
                <button class="dragon-btn dragon-btn-primary" type="button" data-model-action="save">保存模型配置</button>
                <span class="dragon-config-feedback" data-model-feedback role="status" aria-live="polite"></span>
            </div>
        </div>
    `;
}
...
function renderList(state) {
    return state.items.map((item) => `
        <button class="dragon-model-config-list-item" type="button" data-model-id="${escapeAttribute(item.id)}" data-active="${item.id === state.selectedId}">
...
function renderEditor(item, state) {
    if (!item) return '<div class="dragon-empty-state"><p>请新建一个模型配置</p></div>';
    return `
        <div class="dragon-config-section">
            <div class="dragon-config-section-header"><span class="dragon-eyebrow">当前配置</span><h2 class="dragon-config-section-title">${escapeHtml(item.name)}</h2><p class="dragon-config-section-desc">配置名称、模型格式与三个必需组件路径。</p></div>
            <div class="dragon-field-grid-2">
                ${textField('name', '配置名称', item.name, '例如：Krea-2 主模型')}
                ${selectField('model_family', '模型格式', item.model_family, [['anima', 'Anima'], ['krea2_raw', 'Krea-2']])}
            </div>
        </div>
        <div class="dragon-config-section">
            <div class="dragon-config-section-header"><span class="dragon-eyebrow">模型组件</span><h2 class="dragon-config-section-title">基础权重路径</h2><p class="dragon-config-section-desc">支持项目相对路径、绝对路径和环境变量。</p></div>
...
function textField(key, label, value, placeholder) { return `<label class="dragon-field"><span class="dragon-field-label-text">${label}</span><input class="dragon-input" type="text" data-model-field="${key}" value="${escapeAttribute(value)}" placeholder="${escapeAttribute(placeholder)}"></label>`; }
function selectField(key, label, value, options) { return `<label class="dragon-field"><span class="dragon-field-label-text">${label}</span><select class="dragon-select" data-model-field="${key}">${options.map(([option, text]) => `<option value="${option}" ${option === value ? 'selected' : ''}>${text}</option>`).join('')}</select></label>`; }
function showFeedback(root, message, tone) { const el = root.querySelector('[data-model-feedback]'); if (el) { el.textContent = message; el.dataset.tone = tone; el.classList.add('dragon-config-feedback-visible'); } }
function renderError(message) { return `<div class="dragon-page"><div class="dragon-page-hero"><h1>全局模型配置</h1></div><div class="dragon-empty-state"><p>${escapeHtml(message)}</p></div></div>`; }
