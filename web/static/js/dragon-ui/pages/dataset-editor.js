...
import { loadTrainingContext } from './training-controls.js?v=dragon-ui-20260812v35';
...
function renderPage(editor) {
    ...
    return `
        <div class="dragon-page dragon-page-wide dragon-dataset-page">
            <div class="dragon-page-hero dragon-reveal">
                <span class="dragon-eyebrow">数据与标注</span>
                <h1>数据集蓝图</h1>
                <p>配置当前训练来源的图片路径、分桶、验证集和标注规则。保存后会同步写入数据集配置与训练配置。</p>
            </div>
            <section class="dragon-dataset-context dragon-reveal" data-stagger="1">
                ...
                <div><span>数据集文件</span><strong class="dragon-text-mono">${escapeHtml(datasetConfig || '保存时自动生成')}</strong></div>
            </section>
            ${feedback ? `<div class="dragon-config-feedback dragon-config-feedback-visible" data-tone="error">${escapeHtml(feedback)}</div>` : ''}
            <form class="dragon-dataset-form" data-dataset-form>
                <section class="dragon-section dragon-dataset-defaults dragon-reveal" data-stagger="2">
                    <div class="dragon-section-header-row">
                        <div><span class="dragon-eyebrow">默认规则</span><h2 class="dragon-section-title">通用数据设置</h2></div>
                        <span class="dragon-section-desc">每组数据集未单独设置时使用这里的值。</span>
                    </div>
                    ${renderDefaultFields(defaults)}
                </section>
                <section class="dragon-section dragon-dataset-groups dragon-reveal" data-stagger="3">
                    <div class="dragon-section-header-row">
                        <div><span class="dragon-eyebrow">数据来源</span><h2 class="dragon-section-title">数据集分组</h2></div>
                        <button class="dragon-btn dragon-btn-secondary" type="button" data-dataset-add>添加数据集组</button>
                    </div>
                ...
                <div class="dragon-config-actions dragon-config-actions-sticky">
                    <button class="dragon-btn dragon-btn-primary" type="submit" data-dataset-save>保存数据集配置</button>
...
function renderDefaultFields(defaults) {
    return `<div class="dragon-dataset-field-grid">${[
...
function renderRow(row, index, defaults) {
    ...
    return `
        <article class="dragon-dataset-row" data-dataset-row data-index="${index}">
            <header class="dragon-dataset-row-head">
                <div><span class="dragon-eyebrow">数据集组 ${index + 1}</span><h3>训练来源与子集规则</h3></div>
                <button class="dragon-btn dragon-btn-ghost dragon-btn-sm" type="button" data-dataset-remove ${index === 0 ? 'disabled' : ''}>移除</button>
            </header>
...
function textField(key, label, value) {
    return fieldShell(key, label, `<input class="dragon-input" type="text" data-field="${key}" value="${escapeAttribute(value)}">`);
}

function numberField(key, label, value, step = '1') {
    return fieldShell(key, label, `<input class="dragon-input" type="number" step="${step}" data-field="${key}" value="${escapeAttribute(value)}">`);
}

function selectField(key, label, value, options) {
    const selected = typeof value === 'boolean' ? String(value) : String(value ?? '');
    return fieldShell(key, label, `<select class="dragon-select" data-field="${key}">${options.map(([option, text]) => `<option value="${escapeAttribute(option)}" ${option === selected ? 'selected' : ''}>${text}</option>`).join('')}</select>`);
}

function fieldShell(key, label, control) {
    return `<label class="dragon-field" data-field-shell="${key}"><span class="dragon-field-label-text">${label}</span>${control}</label>`;
}
    ...
    root.querySelectorAll('[data-dataset-remove]').forEach((button) => {
        ...
        button.addEventListener('click', () => {
            ...
            root.querySelectorAll('[data-dataset-row]').forEach((row, index) => {
                ...
                const eyebrow = row.querySelector('.dragon-dataset-row-head .dragon-eyebrow');
...
async function saveEditor(root, editor) {
    ...
    try {
        const payload = {
            ...
            defaults: collectFields(root.querySelector('.dragon-dataset-defaults'), editor.defaults),
            config_values: {},
...
function setFeedback(element, message, tone) {
    ...
    element.classList.add('dragon-config-feedback-visible');
}
