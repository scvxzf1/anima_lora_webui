...
| Path | Role |
|------|------|
| `lora_anima/` | LoRA network creation, module targeting, persistence, router diagnostics, optimizer param groups, routing-state orchestration, module builders, merge/pre-calculation helpers, regularization helpers, global routing. Split into `network.py`, `factory.py`, `loading.py`, `config.py`, `persistence.py`, `router_stats.py`, `optimizer_groups.py`, `routing_state.py`, `builders.py`, `merge.py`, `regularization.py`, and `routers.py`. |
