"""LoRA module instantiation helpers for the LoRA-family network facade."""
...
def _effective_spec(nominal_spec, effective_module_class):
    if nominal_spec is not None and effective_module_class is nominal_spec.module_class:
        return nominal_spec
    return next(
        (
            spec
...
def _collect_lora_module_candidates(
    network,
    *,
    is_unet: bool,
...
def _seed_vera_max_dims(cfg: LoRANetworkCfg, nominal_spec, non_skipped) -> None:
    if nominal_spec is None or nominal_spec.name != "vera":
        return
    linear_candidates = [
        child_module
...
def _lora_progress_label(is_unet: bool, text_encoder_idx: Optional[int]) -> str:
    if is_unet:
        return "DiT"
    if text_encoder_idx is not None:
        return f"TE{text_encoder_idx + 1}"
    return "model"


def _resolve_effective_module_class(
    network, module_class, *, is_unet: bool, lora_name: str, original_name: str
):
    if (
        module_class
        in (
        ...
        return _resolve_hydra_module_class(
            network, module_class, lora_name, original_name
        )
    return module_class


def _build_lora_extra_kwargs(
    network,
    *,
    cfg: LoRANetworkCfg,
    nominal_spec,
...
def create_lora_modules(
    ...
    *,
    ...
    non_skipped, skipped = _collect_lora_module_candidates(
        network,
        is_unet=is_unet,
        root_module=root_module,
        target_replace_modules=target_replace_modules,
    ...
    loras = []
    _seed_vera_max_dims(cfg, nominal_spec, non_skipped)

    label = _lora_progress_label(is_unet, text_encoder_idx)
    for lora_name, child_module, dim, alpha_val, original_name in tqdm(
        non_skipped, desc=f"Creating {label} LoRA", leave=False
    ):
        effective_module_class = _resolve_effective_module_class(
            network,
            module_class,
            is_unet=is_unet,
            lora_name=lora_name,
        ...
        extra_kwargs = _build_lora_extra_kwargs(
            network,
            cfg=cfg,
            nominal_spec=nominal_spec,
            effective_module_class=effective_module_class,
            is_unet=is_unet,
            lora_name=lora_name,
            original_name=original_name,
            child_module=child_module,
