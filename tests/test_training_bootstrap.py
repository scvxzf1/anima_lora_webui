...
def test_resolve_sample_prompts_path_via_configs_root(
    ...
    monkeypatch.setattr("library.env.get_configs_root", lambda: configs)

    assert not Path("configs/sample-prompts/imported/demo.txt").is_file()
    resolved = resolve_sample_prompts_path("configs/sample-prompts/imported/demo.txt")
...
def test_normalize_sample_args_keeps_external_configs_path(
    ...
    assert not (out_dir / "sample_prompts.txt").exists()
