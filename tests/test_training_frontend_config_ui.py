...
def test_state_bucket_bridges_reach_hotspot_chunks() -> None:
    ...
    history_workbench_source = _frontend_feature_text("js/features/anima-app/chunks/27-render-history-collections-workbench.js", "js/features/history-list/collections-workbench.js")
    ...
    collection_state_source = _frontend_feature_text("js/features/anima-app/chunks/32-history-task-collection-label.js", "js/features/history-list/task-collections.js")
    history_item_source = _frontend_module_text("js/features/anima-app/chunks/33-create-history-task-item.js")
    history_collection_dialog_source = _frontend_feature_text("js/features/anima-app/chunks/34-show-history-collection-select-dialog.js", "js/features/history-list/task-dialogs.js")
...
def test_config_form_uses_navigation_search_and_progressive_disclosure() -> None:
    ...
    assert "'convrot_hadamard'," in optimization_section
    ...
    assert optimization_section.index("'convrot_scope',") < optimization_section.index("'convrot_hadamard',")
    assert optimization_section.index("'convrot_hadamard',") < optimization_section.index("'block_swap_profile_jsonl',")
    ...
    assert "convrot_hadamard: 'sylvester'" in catalog_defaults
    assert "冻结 DiT 底模 Linear 的计算路径" in catalog_help_training
    assert "ConvRot 分组大小" in catalog_help_training
    assert "Group RHT 的 Hadamard 构造" in catalog_help_training
...
def test_config_training_source_modes_are_audited_before_launch() -> None:
    ...
    tabs_source = _frontend_module_text("js/features/app-shell/tabs.js")
    history_labels = _frontend_feature_text("js/features/anima-app/chunks/32-history-task-collection-label.js", "js/features/history-list/task-collections.js")
...
def test_file_group_drag_has_pointer_fallback() -> None:
    ...
    assert "resolveFileGroupSameListDrop" in source or "moveFileNearList" in source
    assert "moveFileNearList" in source or "configFileDropIndex" in source
...
def test_file_group_drag_matches_history_style_same_list_reorder() -> None:
    """同组排序/预览对齐历史任务：DOM 顺序 + before/after + moveNearList，预览挂父列表。"""
    ...
    # 历史任务同款：相对目标项 before/after 重排，而不是只靠 group.files 下标猜测。
    assert "function moveFileNearList" in core or "export function moveFileNearList" in core
    assert "function fileOrderFromDom" in targets or "function fileGroupOrderFromDom" in targets
    assert "moveFileNearList" in targets
    assert "order: nextOrder" in targets or "order: placeOptions.order" in targets or "body.order" in targets
    assert "anchor: targetFile" in targets or "placeOptions.anchor" in targets or "anchor:" in targets
    assert "position," in targets or "position:" in targets
    assert "position === 'after'" in core or "position === \"after\"" in core

    # 松手结果按 DOM 可视顺序计算最终 index。
    ...
    assert "preview.parentElement !== parent" in core or "parent.appendChild(preview)" in core
...
def test_file_group_drop_targets_resolve_shared_helpers_and_prefer_rows() -> None:
    """拆模块后 targets 依赖齐全，且列表/行落点可工作。"""
    targets = (STATIC_DIR / "js/features/toml-manager/file-group-drag-targets.js").read_text(encoding="utf-8")
    core = (STATIC_DIR / "js/features/toml-manager/file-group-drag-core.js").read_text(encoding="utf-8")
    assert "getDatasetState" in targets
    ...
    assert "resolveFileGroupPointerDropTarget(upEvent.clientX, upEvent.clientY)" in core or "finalX" in core
