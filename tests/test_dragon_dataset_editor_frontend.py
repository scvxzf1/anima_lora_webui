...
def test_dragon_dataset_release_token_is_consistent() -> None:
    token = "dragon-ui-20260814v43"
    index_html = INDEX_HTML.read_text(encoding="utf-8")
    entry = _read("js/dragon-ui/index.js")
    page = _read("js/dragon-ui/pages/dataset-editor.js")
    stylesheet = _read("css/dragon-style.css")

    assert f"dragon-style.css?v={token}" in index_html
    assert f"dragon-ui/index.js?v={token}" in index_html
    assert f"router.js?v={token}" in entry
    assert f"nav.js?v={token}" in entry
    assert f"config-page.js?v={token}" in entry
    assert f"dataset-editor.js?v={token}" in entry
    assert page.count(token) >= 6
    assert stylesheet.count(token) == 7
