"""Global settings for image-test discovery and output paths."""
...
def _patch_model_path_resolution(monkeypatch) -> None:
    monkeypatch.setattr(
        image_test_service,
...
def _minimal_image_test_request(**overrides):
    return {
        "prompt": "a cat",
        "config": {
            "pretrained_model_name_or_path": "models/dit.safetensors",
...
def test_image_test_allow_home_search_roundtrip(tmp_path, monkeypatch):
    ...
    assert settings_service.get_global_settings()["image_test_allow_home_search"] is True
...
def test_empty_save_root_falls_back_to_output_tests(tmp_path, monkeypatch):
    ...
    assert got.get("image_test_save_root", "") in {"", None}
...
def test_normalize_request_uses_save_root_default(tmp_path, monkeypatch):
    ...
    _patch_model_path_resolution(monkeypatch)

    out = image_test_service._normalize_image_test_request(_minimal_image_test_request())
...
def test_explicit_request_save_path_wins(tmp_path, monkeypatch):
    ...
    out = image_test_service._normalize_image_test_request(
        _minimal_image_test_request(save_path="output/other")
    )
