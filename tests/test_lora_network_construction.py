...
def test_dora_initial_forward_matches_base_linear() -> None:
    ...
    with torch.no_grad():
        torch.testing.assert_close(base(inputs), reference(inputs), rtol=1e-6, atol=1e-6)


def test_dora_fuse_unfuse_restores_original_weight() -> None:
