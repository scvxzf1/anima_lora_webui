...
This implementation is the **α-adaptive variant** of the paper — the fixed switching gain `k` is replaced with `k_t = α · mean(|e_t|)` per step (cf. Plestan et al. 2010, adaptive sliding-mode control). The paper's fixed `k=0.1` was empirically off by ~14× on Anima at CFG=4 (see the [archived analysis](../../_archive/docs/findings/smc-cfg-analysis-and-proposal-202605.md)) and produced visible chattering; the α form self-scales across model / CFG / σ / sample.
...
The historical bench measurement, summarized in the [archived analysis](../../_archive/docs/findings/smc-cfg-analysis-and-proposal-202605.md), shows `mean(|e_t|)` on Anima at CFG=4 is roughly two orders of magnitude smaller than what the paper's fixed `k=0.1` assumes. Concretely, the paper's value over-injects the correction by ~14× and produces visible texture noise. The original `bench/smc_cfg/` harness was intentionally retired in commit `f2c5f72f`; it is not a current runnable path.
...
The checked-in plot was generated by the retired `bench/smc_cfg/` harness and is retained as historical evidence; the derivation and measurement context are preserved in the archived analysis linked above.
...
| [archived SMC-CFG analysis](../../_archive/docs/findings/smc-cfg-analysis-and-proposal-202605.md) | Frozen design analysis and measurement context for the α-adaptive form. The original bench scripts were retired in commit `f2c5f72f`. |
