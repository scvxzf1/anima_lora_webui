...
| 阶段 | 目标 | 退出条件 | 依赖 |
|---|---|---|---|
| **0 可行性确认** | 跑通官方推理 + dump key + 验证 VAE encode 互逆 | transformer state_dict key 清单到手；`AutoencoderKLQwenImage` encode→decode 互逆无损 | HF license + ~33GB 下载 |
...
| **2 DiT 本体 + 加载器** | `library/models/krea2_raw/dit.py` + `weights.py`，diffusers key 映射 | 单 latent forward 通过，shape 对齐 | 阶段 0 |
| **3 LoRA 注入** | `library/models/krea2_raw/lora_targets.py`，注入点按 single-stream 重做 | 单 block LoRA attach + forward 正常 | 阶段 2 |
