...
- `Krea2PipelineParallelConfig` 负责 CLI、TOML、WebUI 共用的归一化；默认 microbatch 为 4，
  可配置范围为 1--1024，首轮建议 4--8。
...
- 共享兼容矩阵检查阶段数、调度、分层策略以及与 block swap、compile、
  selective/CPU/Unsloth offload 的组合；CLI 还会用实际 `WORLD_SIZE` 校验进程拓扑。
