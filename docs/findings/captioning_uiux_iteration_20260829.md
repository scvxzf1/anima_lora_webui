...
## 长期专项阶段 07：日志、配置与 API 设置（2026-08-29）

### 本阶段落地

- `logs-panel.js`：日志筛选空态增加原位“重新加载”入口，加载错误和无数据不再需要回到顶部操作。
- `settings-panel.js`：API Key 默认继续隐藏，增加显式显示/隐藏按钮；保存和测试前校验重试、并发、超时参数均为有效数字。
- `06c-dragon-captioning.css`：为密钥输入和切换按钮增加稳定的横向布局，移动端不挤压输入框。
- `tests/test_dragon_captioning_frontend.py`：补充密钥切换、数值校验和日志空态契约。

### 阶段验收

- 两个模块 `node --check` 通过。
- Captioning 前端、工作区、路由定向测试：`29 passed`。
- 至此 13 个界面均至少完成一项基于截图/源码证据的专项改动；剩余风险集中在真实大数据量下的性能和交互节奏，不再以“未改代码”冒充完成。

## 长期专项阶段 08：13 面板最终验收（2026-08-29）

### 配置中心补强

- `config-panel.js`：危险区按钮明确标注“不可撤销”，并同步说明执行前需要两次确认，降低清空工作区、渠道与调度的误触概率。

### 最终验收证据

- 13 个面板逐一在 Playwright Chrome `390×844` 切换，横向溢出全部为 `0`。
- API / 调度设置对话框密钥切换：`password -> text` 正常，横向溢出为 `0`。
- 浏览器控制台错误：`[]`。
- 完整定向测试：`54 passed`。
- 配置中心模块 `node --check` 通过。

本阶段完成的是有截图和测试证据支撑的优化范围；大数据量性能、真实 API 失败率和人工可用性测试仍属于独立后续工作，不计入本次完成声明。
