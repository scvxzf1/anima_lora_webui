...
| `ANIMA_BLOCK_SWAP_PREFETCH_DEPTH` | `library/runtime/block_swap_config.py`、`library/runtime/offloading.py` | 训练路径块交换预取深度 K（env 变量） | 默认 `2`，`max(1,…)`；推理（forward-only）固定 `1` | 传输略超单块计算时（RTX 3080 bf16 overlap 0.878）把 H2D 藏进多块计算 | K≥2 时 worker rebinding 非原子，极小 toy 模型可能读到 CPU 权重（真实 Anima 模型不触发）；设 `1` 退回旧行为 | CLI/env |
